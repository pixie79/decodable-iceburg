...
import static co.za.projectolympus.recon.StreamingJob.SIMPLE_EVENTS_STREAM;

@SourceStreams(SIMPLE_EVENTS_STREAM)
public class StreamingJob {
    ...
    private static final String DEFAULT_ICEBERG_SORT_ORDER_FIELD = "outbox_published_date";
    private static final String DEFAULT_ICEBERG_PARTITION_FIELDS = "topic_name,outbox_published_date";
...
//    }
    ...
    private static FlinkSink.Builder createIcebergSinkBuilder(ParameterTool applicationProperties,DataStream<GenericRecord> dataStream, Schema avroSchema) {
        ...
        String prefix = applicationProperties.get("prefix", PREFIX);
        String warehouse = "s3://" +
                applicationProperties.get("aws.account_id", AWS_ACCOUNT_ID) + "-" +
                applicationProperties.get("aws.region", DEFAULT_AWS_REGION) + "-" +
                applicationProperties.get("iceberg.warehouse", DEFAULT_ICEBERG_S3_BUCKET) + "-" +
                applicationProperties.get("prefix", PREFIX) + "_" +
                applicationProperties.get("iceberg.table", DEFAULT_ICEBERG_TABLE_NAME);
        String s3Endpoint = "https://s3." + applicationProperties.get("aws.region", DEFAULT_AWS_REGION) + "amazonaws.com";
        ...
        TableIdentifier outputTable = TableIdentifier.of(
                ...
                applicationProperties.get("prefix", PREFIX) + "_" + applicationProperties.get("iceberg.table",DEFAULT_ICEBERG_TABLE_NAME));
    ...
    public static void main(String[] args) throws Exception {
        ...
        DecodableStreamSource<String> source =
                DecodableStreamSource.<String>builder()
                        .withStreamName(applicationProperties.get("prefix", PREFIX) + "-" + SIMPLE_EVENTS_STREAM)
                        .withDeserializationSchema(new SimpleStringSchema())
                        .build();

        DataStream<String> input = env.fromSource(source, WatermarkStrategy.noWatermarks(), "TenX Simple Events combined Stream");
